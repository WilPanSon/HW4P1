...
class SequenceGenerator:
    ...
    def generate_beam(
        ...
        batch_size = x.size(0)
        seq_len = x.size(1)
        vocab_size = self.tokenizer.vocab_size if hasattr(self.tokenizer, 'vocab_size') else self.score_fn(x).shape[-1]
        ...
        for _ in range(self.max_length - seq_len):
            ...
            inp = sequences.view(-1, sequences.size(-1))
            
            logits = self.score_fn(inp)
            ...
            beam_indices = torch.div(top_indices, vocab_size, rounding_mode='floor') # (batch, beam)
            token_indices = top_indices % vocab_size # (batch, beam)

            new_sequences = torch.zeros(batch_size, beam_width, sequences.size(1) + 1, device=x.device, dtype=x.dtype)
            new_finished = torch.zeros_like(finished)

            for b in range(batch_size):

                parent_seqs = sequences[b][beam_indices[b]]
                new_tokens = token_indices[b].unsqueeze(1) # (beam, 1)
