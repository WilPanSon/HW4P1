...
class ASRTrainer(BaseTrainer):
    ...
    def _get_time_reduction(self) -> int:
        for attr in ["time_reduction", "time_reduction_factor"]:
            if hasattr(self.model, attr):
                ...
                if isinstance(tr, int) and tr >= 1: return tr
        if hasattr(self.model, "speech_embedding"):
            ...
            for attr in ["time_reduction", "time_reduction_factor"]:
                if hasattr(se, attr):
                    tr = getattr(se, attr)
                    if isinstance(tr, int) and tr >= 1: return tr
        tr = self.config.get("model", {}).get("speech_embedding", {}).get("time_reduction", 1)
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=device_type, dtype=amp_dtype):
                # Using unpacking with ignore for potential extra returns
                seq_out, curr_att, ctc_inputs, *extras = self.model(
                ...
                if self.ctc_weight > 0:
                    if isinstance(ctc_inputs, dict) and 'log_probs' in ctc_inputs:
                        ...
                        ctc_lens = torch.div(feat_lengths, time_reduction, rounding_mode="floor").clamp(min=1, max=log_probs.size(0))
                    ctc_loss = self.ctc_criterion(log_probs, targets_golden, ctc_lens, transcript_lengths)
            ...
            if self.ctc_weight > 0: running_ctc_loss += ctc_loss.item() * batch_tokens
            running_joint_loss += loss.item() * batch_tokens

            loss = loss / self.config['training']['gradient_accumulation_steps']
            if self.scaler is not None: self.scaler.scale(loss).backward()
            else: loss.backward()
            ...
            batch_bar.set_postfix(
                ce_loss=f"{avg_ce_loss:.4f}", ctc_loss=f"{avg_ctc_loss:.4f}",
                joint_loss=f"{avg_joint_loss:.4f}", perplexity=f"{perplexity:.4f}",
                acc_step=f"{(i % self.config['training']['gradient_accumulation_steps']) + 1}/{self.config['training']['gradient_accumulation_steps']}"
            )
            batch_bar.update()
            del feats, targets_shifted, targets_golden, feat_lengths, transcript_lengths, seq_out, curr_att, ctc_inputs, loss
        ...
        batch_bar.close()
        return {'ce_loss': avg_ce_loss, 'ctc_loss': avg_ctc_loss, 'joint_loss': avg_joint_loss, 'perplexity_token': avg_perplexity_token.item(), 'perplexity_char': avg_perplexity_char.item()}, running_att

    def _validate_epoch(self, dataloader):
        full_greedy_cfg = {'num_batches': None, 'beam_width': 1, 'temperature': 1.0, 'repeat_penalty': 1.0, 'lm_weight': 0.0, 'lm_model': None}
    ...
    @torch.no_grad()
    def evaluate(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: str = "greedy", max_length: Optional[int] = None):
        if max_length is None:
            if hasattr(self, "text_max_len"): max_length = self.text_max_len
            elif hasattr(dataloader.dataset, "text_max_len"): max_length = dataloader.dataset.text_max_len
        if recognition_config is None:
            ...
            config_name = "greedy"
        results = self.recognize(dataloader, recognition_config=recognition_config, config_name=config_name, max_length=max_length)
        references = [r['target'] for r in results if 'target' in r]
        hypotheses = [r['generated'] for r in results if 'target' in r]
        if references and hypotheses: metrics = self._calculate_asr_metrics(references, hypotheses)
        else: metrics = {}
        return metrics, results
    ...
    def train(self, train_dataloader, val_dataloader, epochs: int):
        ...
        if self.optimizer is None: raise ValueError("Optimizer is not initialized, initialize it first!")
        ...
        for epoch in range(self.current_epoch, self.current_epoch + epochs):
            ...
            if train_attn_keys:
                ...
                if decoder_self_keys:
                    ...
                    if first_self_key in train_attn: self._save_attention_plot(train_attn[first_self_key][0], epoch, "decoder_self")
                if decoder_cross_keys:
                    last_cross_key = decoder_cross_keys[-1]
                    if last_cross_key in train_attn: self._save_attention_plot(train_attn[last_cross_key][0], epoch, "decoder_cross")
    ...
    def recognize(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: Optional[str] = None, max_length: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        if recognition_config is None:
            recognition_config = {'num_batches': 5, 'beam_width': 1, 'temperature': 1.0, 'repeat_penalty': 1.0, 'lm_weight': 0.0, 'lm_model': None}
            config_name = 'greedy'
        if recognition_config.get('lm_model') is not None:
            recognition_config['lm_model'].eval()
            recognition_config['lm_model'].to(self.device)

        generator = SequenceGenerator(score_fn=None, tokenizer=self.tokenizer, max_length=max_length if max_length is not None else self.text_max_len, device=self.device)
        self.model.eval()
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                feats = feats.to(self.device)
                feat_lengths = feat_lengths.to(self.device)
                if targets_golden is not None: targets_golden = targets_golden.to(self.device)
                ...
                if recognition_config['beam_width'] > 1 and hasattr(generator, "generate_beam"):
                    seqs, scores = generator.generate_beam(
                        prompts, beam_width=recognition_config.get('beam_width', 1),
                    ...
                    seqs, scores = generator.generate_greedy(
                        prompts, temperature=recognition_config.get('temperature', 1.0),
                        repeat_penalty=recognition_config.get('repeat_penalty', 1.0)
                    )
                ...
                if targets_golden is not None:
                    ...
                    for j, (pred, target) in enumerate(zip(post_processed_preds, post_processed_targets)):
                        results.append({'target': self.tokenizer.decode(target.tolist(), skip_special_tokens=True), 'generated': self.tokenizer.decode(pred.tolist(), skip_special_tokens=True), 'score': scores[j].item()})
                else:
                    for j, pred in enumerate(post_processed_preds):
                        results.append({'generated': self.tokenizer.decode(pred.tolist(), skip_special_tokens=True), 'score': scores[j].item()})

                batch_bar.update()
                if recognition_config['num_batches'] is not None and i >= recognition_config['num_batches'] - 1: break
        batch_bar.close()
        return results
    ...
    def _calculate_asr_metrics(self, references, hypotheses):
        ...
        return {'wer': wer(hypotheses, references).item() * 100, 'cer': cer(hypotheses, references).item() * 100}
...
class ProgressiveTrainer(ASRTrainer):
    ...
    def configure_stage(self, stage_config):
        ...
        self.ce_criterion = nn.CrossEntropyLoss(ignore_index=self.tokenizer.pad_id, label_smoothing=stage_config['label_smoothing'])
        ...
        for i, freeze in enumerate(encoder_freeze):
            for param in self.model.enc_layers[i].parameters(): param.requires_grad = not freeze
        for i, freeze in enumerate(decoder_freeze):
            for param in self.model.dec_layers[i].parameters(): param.requires_grad = not freeze

    def get_subset_dataloader(self, dataloader, subset_ratio):
        if subset_ratio >= 1.0: return dataloader
        subset_size = int(len(dataloader.dataset) * subset_ratio)
        subset = Subset(dataloader.dataset, list(range(subset_size)))
        subset.text_max_len = dataloader.dataset.text_max_len
        subset.get_avg_chars_per_token = dataloader.dataset.get_avg_chars_per_token
        return torch.utils.data.DataLoader(subset, batch_size=dataloader.batch_size, shuffle=True, num_workers=dataloader.num_workers, collate_fn=dataloader.collate_fn, pin_memory=dataloader.pin_memory)

    def transition_to_full_training(self):
        print("\n" + "="*80); print("Transitioning to Full Model Training".center(80)); print("="*80)
    ...
    def progressive_train(self, train_dataloader, val_dataloader):
        ...
        for i, stage_config in enumerate(stages):
            ...
            stage_loader = self.get_subset_dataloader(train_dataloader, stage_config['data_subset'])
