...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.dec_layers     = nn.ModuleList([SelfAttentionDecoderLayer(d_model=d_model, num_heads=num_heads, d_ff=d_ff, dropout=dropout)
    ...
    def forward(self, padded_targets: torch.Tensor, target_lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, dict]:
        ...
        # Create causal mask to prevent attending to future tokens on the same device as the input
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.enc_layers = nn.ModuleList([SelfAttentionEncoderLayer(d_model=d_model, num_heads=num_encoder_heads, d_ff=d_ff_encoder, dropout=dropout)
            for _ in range(num_encoder_layers)])

        # Create decoder layers
        self.dec_layers = nn.ModuleList([CrossAttentionDecoderLayer(d_model=d_model, num_heads=num_decoder_heads, d_ff=d_ff_decoder, dropout=dropout)
            for _ in range(num_decoder_layers)])

        # Create source and target embeddings and other layers
        self.source_embedding = SpeechEmbedding(input_dim=input_dim, output_dim=d_model, time_reduction=time_reduction, reduction_method=reduction_method, dropout=dropout)
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        ctc_logits = {
            'log_probs': self.ctc_head(x_enc).transpose(0, 1),
    ...
    def decode(
        ...
        for i in range(self.num_decoder_layers):
            if self.training and self.layer_drop_rate > 0 and random.random() < self.layer_drop_rate:
                continue
            # Pass through decoder layer
            x_dec, self_attn, cross_attn = self.dec_layers[i](x_dec, encoder_output, pad_mask_tgt, pad_mask_src, attn_mask=causal_mask)
    ...
    def forward(
        ...
        seq_out, dec_running_att = self.decode(padded_targets, encoder_output, target_lengths=target_lengths, pad_mask_src=pad_mask_src)
