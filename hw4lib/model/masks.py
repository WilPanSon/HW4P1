...
def PadMask(padded_input, input_lengths):
    ...
    # Create a range of indices [0, 1, ..., T-1]
    # Shape: (1, T) to allow broadcasting
    indices = torch.arange(T, device=padded_input.device).unsqueeze(0)
    
    # Reshape lengths to (N, 1) to allow broadcasting
    # Ensure lengths are on the same device for comparison
    lengths = input_lengths.to(padded_input.device).unsqueeze(1)
    
    # Create the mask
    # If the index is >= the length, it is a padding position (True)
