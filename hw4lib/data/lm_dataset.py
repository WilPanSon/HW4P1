...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        possible_text_dir = os.path.join(config['root'], partition, 'text')
        possible_transcript_dir = os.path.join(config['root'], partition, 'transcript')

        if os.path.exists(possible_text_dir) and len(os.listdir(possible_text_dir)) > 0:
            self.text_dir = possible_text_dir
        elif os.path.exists(possible_transcript_dir) and len(os.listdir(possible_transcript_dir)) > 0:
            self.text_dir = possible_transcript_dir
        else:
            # Fallback to the partition root if subfolders don't exist
            self.text_dir = os.path.join(config['root'], partition)

        print(f"Dataset looking for files in: {self.text_dir}") # Debug print
