...
class MultiHeadAttention:
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        :param attn_mask: (L, S)
        :return: (N, H, L, S) (implicitly via broadcasting)
        """
        key_mask = None
        attention_mask = None

        # Expand key_padding_mask to (N, 1, 1, S)
        if key_padding_mask is not None:
            key_mask = key_padding_mask[:, None, None, :]
        
        # Expand attn_mask to (1, 1, L, S)
        if attn_mask is not None:
            attention_mask = attn_mask[None, None, :, :]
        
        # Combine masks using logical_or
    ...
    def _split_heads(self, x):
        ...
        x = x.reshape(batch_size, length, self.num_heads, head_dim)
    ...
    def _concat_heads(self, x):
        ...
        x = x.reshape(batch_size, length, self.embed_dim)
